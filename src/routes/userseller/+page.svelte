<script>
  ...
  async function fetchData() {
    try {
      const response = await fetch("http://localhost:9999/api/userallseller");
      if (!response.ok) {
  ...
  async function Register() {
    ...
    let url = `http://localhost:9999/api/registerseller/create`;
  ...
  async function Delete(id) {
    ...
    let url = `http://localhost:9999/api/deleteseller?id_seller=${id}`;
    try {
      const response = await fetch(url, requestOptions);
  ...
  async function Update(userid) {
    ...
    let url = `http://localhost:9999/api/updateseller?id_seller=${userid}`;
    const response = await fetch(url, requestOptions);
  ...
  async function Getuserid(id) {
    ...
    let url = `http://localhost:9999/api/useraidseller?id_seller=${id}`;
    const response = await fetch(url, requestOptions);
    seller = await response.json();
    console.log(seller);
    if (id) {
    ...
    <div class="container">
      <div class="teble-head" style="margin-top: 90px;">
        ...
        <form class="d-flex search-form">
          ...
          >
            <label for="submit" on:click={() => Getuserid(id)}>
          ...
          <button
            ...
            style="margin-left: 10px;"
          >
            <label for="submit" on:click={() => Getuserid()}>
      ...
      <table class="table" style="width: 100%; margin-top: 10px;">
        ...
        <tbody>
          ...
          {#each seller as user}
            <tr>
              <td>{user.Id_seller}</td>
              <td>{user.Code_shop}</td>
              <td>{user.Name_shop}</td>
              <td>{user.Name_seller} </td>
              <td>{user.Email_seller}</td>
              <td>{user.Phone_seller}</td>
              {#if user.Status}
              ...
              <td>
                ...
                <svg
                  on:click={() => {
                    modal_update = true;
                    formdata.Id_seller = user.Id_seller;
                    formdata.Name_shop = user.Name_shop;
                    formdata.Name_seller = user.Name_seller;
                    formdata.Address_seller = user.Address_seller;
                    formdata.Email_seller = user.Email_seller;
                    formdata.Password_seller = formdata.Password_seller;
                    formdata.ConfirmPassword = formdata.ConfirmPassword;
                    formdata.Phone_seller = user.Phone_seller;
...
{#if modal_create}
  <div class="modal">
    <div class="modal-content">
      <div class="d-flex justify-content-between">
        <h3>{modal_create ? "เพิ่มผู้ขาย" : ""}</h3>
        <!-- svelte-ignore a11y-no-static-element-interactions -->
        <!-- svelte-ignore a11y-click-events-have-key-events -->
        <span
          class="close"
      ...
      <input
        ...
        bind:value={formdata.Name_seller}
      />
      <span class="mb-2">รหัสร้านค้า :</span>
      <input
        type="text"
        class="form-control mb-2"
        bind:value={formdata.Code_shop}
      />
      <span class="mb-2">ชื่อร้านค้า : </span>
      <input
        type="text"
        class="form-control mb-2"
        bind:value={formdata.Name_shop}
      />
      <span class="mb-2">ที่อยู่ :</span>
      <input
        type="text"
        class="form-control mb-2"
        bind:value={formdata.Address_seller}
      />
      <span class="mb-2">เบอร์โทรศัทพ์ :</span>
      <input
        type="text"
        class="form-control mb-2"
        bind:value={formdata.Phone_seller}
        maxlength="10"
      />
      ...
      <input
        type="Email"
        class="form-control mb-2"
        bind:value={formdata.Email_seller}
      />
      <span class="mb-2">รหัสผ่าน :</span>
      <input
        type="pasword"
        class="form-control mb-2"
        bind:value={formdata.Password_seller}
      />
